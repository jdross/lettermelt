...
letters. The app lives at the repository root, with build utilities under
...
`.vial` in `styles.css`: one clipped rounded bore, a five-stop lava gradient,
...
The result sheet is shared by wins and losses. Desktop copies the challenge
text to the clipboard and temporarily confirms with “copied!”. Mobile opens an
SMS composer with the challenge and deterministic puzzle link pre-filled. Keep
the desktop/mobile split explicit: desktop browsers may expose Web Share even
though the intended desktop action is copying.
