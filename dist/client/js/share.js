...
(function (root, factory) {
  ...
  function createController(options) {
    ...
    const labelElement = opts.labelElement || button;
    const nav = opts.navigator || {};
    const doc = opts.document;
    const win = opts.window;
    const defaultLabel = opts.defaultLabel || 'Share with friends';
    const copiedLabel = opts.copiedLabel || 'copied!';
    const mobileShare = opts.mobileShare !== false;
    let resetTimer = null;
    ...
    function share() {
      ...
      if (mobileShare && isMobileDevice(nav)) {
