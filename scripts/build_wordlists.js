 ...
 *
 * Generates:
 *   - data/common.js   (window.LETTER_MELT_COMMON = [...] lengths 4-7,
 *                       window.LETTER_MELT_COMMON_LONG = [...] lengths 8-11)
 *   - data/dict.js     (window.LETTER_MELT_DICT_RAW = "word word ...",
...
// dominating). Cap how deep we'll walk it for LETTER_MELT_COMMON_LONG specifically,
...
// ---------------------------------------------------------------------------
// Blocklist: offensive / slur terms filtered out of LETTER_MELT_COMMON (and, to be
...
function pickCommonWords(dictSet, stemSet, tiers, isBlocked, zipf) {
  ...
  if (freq && !tiers) {
    ...
    // LETTER_MELT_COMMON, longer-but-still-frequent words feed LETTER_MELT_COMMON_LONG. This
    // keeps LETTER_MELT_COMMON_LONG sourced from the same "words an average player
  ...
  // LETTER_MELT_COMMON_LONG has no fallback word list to top up from (the curated
...
function toJsFileCommon(words, wordsLong, wordsBase, easy) {
  ...
  const e = easy || {};
  return (
    '// Auto-generated by scripts/build_wordlists.js — do not edit by hand.\n' +
    ...
    'window.LETTER_MELT_COMMON = ' + json + ';\n\n' +
    ...
    "// used for LETTER_MELT_COMMON, so they should still be words an average player\n" +
    '// knows (e.g. birthday, elephant, chocolate, dangerous, basketball).\n' +
    'window.LETTER_MELT_COMMON_LONG = ' + jsonLong + ';\n\n' +
    ...
    'window.LETTER_MELT_BASE = ' + jsonBase + ';\n\n' +
    ...
    'window.LETTER_MELT_COMMON_EASY = ' + JSON.stringify(e.words || []) + ';\n' +
    'window.LETTER_MELT_LONG_EASY = ' + JSON.stringify(e.wordsLong || []) + ';\n' +
    'window.LETTER_MELT_BASE_EASY = ' + JSON.stringify(e.wordsBase || []) + ';\n'
  );
}
...
function toJsFileDict(words, maxLen) {
  ...
  return (
    ...
    'window.LETTER_MELT_DICT_RAW = ' + JSON.stringify(raw) + ';\n'
  );
}
...
function main() {
  ...
  // dict set (LETTER_MELT_COMMON and LETTER_MELT_COMMON_LONG are disjoint by length range).
  ...
  // cap it at length 10 instead, but always keep the LETTER_MELT_COMMON_LONG words
  ...
  console.log('  common.js : LETTER_MELT_COMMON=%d words, LETTER_MELT_COMMON_LONG=%d words, %d bytes -> %s',
  ...
  if (longToppedUpFromDict > 0) {
    console.log('  NOTE: %d LETTER_MELT_COMMON_LONG words topped up from raw dictionary (not frequency-ranked) because the frequency source ran short at lengths 8-11.', longToppedUpFromDict);
  }
}
